...
async def create_set_items(
    ...
    payload = CreateItemBuild(
        hero_id=hero_id,
        item_ids=[item1, item2, item3, item4, item5, item6]
    )
    # สมมติเก็บลง collection ชื่อ build_sets
    await db["build_sets"].insert_one(payload.dict())
    return RedirectResponse("/newsetitems", status_code=303)
